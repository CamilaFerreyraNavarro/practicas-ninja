 ...
 class NinjaPlayerAnswer(models.Model):
	...
	is_orrect = fields.Boolean(string='¿Correcta?', compute='_compute_is_corrrect', store=True)
	...
	def _compute_is_correct(self):
		for record in self: 
		    record.is_correct = record.answer = record.answer_id.is_correct

	@api.depends('answer_id')
	def _compute_points(self):
		for record in self:
		    record.points_awarded = record.answer_id.pints if record.answer_id.is_correct else 0
